...
export default function StocksHeatmap() {
  ...
  const treeData = useMemo(() => {
    // 1) aggregate by instrument (combine multiple accounts/brokers)
    type Agg = {
      instrument_id: number;
      symbol: string | null;
      name: string | null;
    ...
    const byInstrument: Record<number, Agg> = {};

    for (const r of rows) {
      const inst = instMap[r.instrument_id];
      const sector =
        inst?.sector?.trim() || (r.asset_subclass ? r.asset_subclass : "Other");

      const mv = Number(r.market_value_base || 0);
      const cost = Number((r.avg_cost_base || 0) * (r.qty || 0));
      ...
      const agg = byInstrument[r.instrument_id]!;
      agg.totalMv += mv;
      agg.totalCost += cost;
      agg.totalPnl += pnl;
    }

    // 2) group instruments by sector
    const bySector: Record<
      string,
      { name: string; children: any[]; total: number }
    > = {};

    for (const agg of Object.values(byInstrument)) {
      const pnlPct = agg.totalCost > 0 ? agg.totalPnl / agg.totalCost : 0;

      if (!bySector[agg.sector]) {
        bySector[agg.sector] = { name: agg.sector, children: [], total: 0 };
      }
      const sectorNode = bySector[agg.sector]!;

      const ticker =
        (agg.symbol && String(agg.symbol)) ||
        (agg.name && String(agg.name)) ||
        String(agg.instrument_id);

      sectorNode.children.push({
        id: agg.instrument_id, // unique identity for Nivo
        name: ticker, // label only
        ticker,
        value: Math.max(0, agg.totalMv), // size encodes MV
        mv: agg.totalMv,
        pnl: agg.totalPnl,
        pnlPct,
        sector: agg.sector,
      ...
      sectorNode.total += Math.max(0, agg.totalMv);
    }
    ...
    return (
      <Paper sx={{ p: 1.25, bgcolor: "#0f172a", color: "#fff", borderRadius: 1.5 }}>
        ...
        <Stack spacing={0.5} sx={{ mt: 0.75, fontSize: 12 }}>
          <div>
            <b>Market Value:</b> {d.mv?.toLocaleString()} {baseCcy}
          </div>
          <div>
    ...
    <Box sx={{ height: "100%", display: "flex", flexDirection: "column" }}>
      ...
      >
        <Box>
          <Typography variant="h6" sx={{ fontWeight: 800 }}>
            ...
            >
              size = Market Value ({baseCcy}), color = P&amp;L% vs cost (green
              gain, red loss)
            </Typography>
          </Typography>
        </Box>
          ...
          <Box sx={{ display: "flex", alignItems: "center", gap: 1 }}>
            ...
            <Box
              sx={{
                ...
                borderRadius: 999,
                background:
                  "linear-gradient(90deg, rgb(220,38,38) 0%, rgb(96,105,120) 50%, rgb(16,185,129) 100%)",
      ...
      {!loading && !err && treeData.children && treeData.children.length > 0 && (
        <Paper sx={{ flex: 1, minHeight: 560, p: 1.5 }}>
          <ResponsiveTreeMap
            data={treeData}
            identity="id"           // use the unique id we provided
            value="value"
            tile="squarify"
            innerPadding={5}
            outerPadding={6}
            ...
            label={(node: any) => {
              const d = node.data;
              const pctStr =
                typeof d.pnlPct === "number"
                  ? `${Math.round(d.pnlPct * 100)}%`
                  : "";
              ...
              if (minSide < 42) return "";
              if (minSide < 76) return ticker;
              return pctStr ? `${ticker} · ${pctStr}` : ticker;
            }}
            labelSkipSize={40}
            orientLabel={false}
