...
def setup_verify(
    ...
    for old_code in session.exec(select(RecoveryCode).where(RecoveryCode.user_id == user.id)).all():
        session.delete(old_code)
    
    for _ in range(10):
        # Generate 10-character recovery code (well under bcrypt's 72-byte limit)
        rc = "".join(secrets.choice("ABCDEFGHJKLMNPQRSTUVWXYZ23456789") for _ in range(10))
        codes_to_return.append(rc)
        session.add(RecoveryCode(user_id=user.id, code_hash=bcrypt.hash(rc)))
    session.commit()

    # mark session as 2fa_ok now, respect remember flag
    remember_flag = str(remember).lower() in ("1", "true", "yes")
    remember_days = settings.SESSION_REMEMBER_DAYS if remember_flag else None
    cookie = create_session_cookie(user_id=user.id, twofa_ok=True, remember_days=remember_days)
    resp = JSONResponse({"ok": True, "recovery_codes": codes_to_return})
    set_cookie(resp, cookie, remember=remember_flag)
    return resp
...
def verify_login(
    ...
    if not ok:
        ...
        used = False
        for rc in codes:
            if bcrypt.verify(body.code, rc.code_hash):
